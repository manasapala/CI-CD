...
resource "aws_instance" "app" {
  ...
  provisioner "local-exec" {
    command = "sleep 120; ANSIBLE_HOST_KEY_CHECKING=False /Library/ansible/ansible-2.8.2/bin/ansible-playbook -u ${var.ssh_user} --private-key ~/.ssh/jenkins_west_2.pem -i '${self.private_ip}' '../../ansible/site.yml'"
  }
